...
class Lab6 extends GLJPanel implements GLEventListener  {
    ...
    private void createPyramid(GL2 gl2){
        ...
        for (int i = 1; i <= 9; i++) {
            ...
            gl2.glMaterialfv (GL2.GL_FRONT, GL2.GL_AMBIENT_AND_DIFFUSE, materials[i], 1);
            gl2.glVertex3f(x, -2.0f, z);

            gl2.glMaterialfv(GL2.GL_FRONT, GL2.GL_AMBIENT_AND_DIFFUSE, materials[i],2);
            gl2.glVertex3f( 0.0f, 7.0f, 0.0f );

            angle = i * angleIncrement;

            gl2.glMaterialfv(GL2.GL_FRONT, GL2.GL_AMBIENT_AND_DIFFUSE, materials[i],3);
            gl2.glVertex3f((float) (radius * Math.cos(angle)), -2.0f,(float) (radius * Math.sin(angle)));
            gl2.glEnd();
        }
