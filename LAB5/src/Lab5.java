...
class Lab5 extends GLJPanel implements GLEventListener, KeyListener {
    ...
    private void createCorkscrew(GL2 gl2, double radius, int turnsAmount) {
        gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);

        int numVertices = turnsAmount * 1000;
        double angleIncrement = turnsAmount * Math.PI / numVertices;
        float height = 0;
        ...
        for (int i = 1; i <= numVertices; i++) {
            ...
            height += turnsAmount / 3000f;
        }
        gl2.glEnd();

        gl2.glTranslated(0, height, -4.5);
        glu.gluCylinder(glu.gluNewQuadric(), 1.3, 1.3, 9, 100, 50);
    }

    private void CreatePiramid(GL2 gl2, double size) {
        ...
        for (int i = 0; i < anglesNumber; i++) {
            ...
            gl2.glVertex3f((float) (Math.cos(i * 2 * Math.PI / anglesNumber) * size), 0, (float) (Math.sin(i * 2 * Math.PI / anglesNumber) * size));
            gl2.glColor3f(0.0f, 1.0f, 0.0f);
            gl2.glVertex3f((float) (Math.cos((i + 1) * 2 * Math.PI / anglesNumber) * size), 0, (float) (Math.sin((i + 1) * 2 * Math.PI / anglesNumber) * size));
        }
        gl2.glEnd();
        gl2.glBegin(gl2.GL_POLYGON);
